# Svelte SPA Router Showcase - Makefile
# Development and build commands for the showcase project

# === Configuration ===
# Docker image settings
DOCKER_IMAGE_NAME = registry.km8.es/svelte-spa-router-showcase
DOCKER_TAG = production
DOCKER_CONTAINER_NAME = svelte-spa-router-showcase
DOCKER_PORT = 8081
...
help: ## Show this help message
	@echo "Svelte SPA Router Showcase - Available Commands:"
	@echo ""
...
link-lib: ## Link the @keenmate/svelte-spa-router library from ../svelte-spa-router
	cd ../svelte-spa-router && npm link
	npm link @keenmate/svelte-spa-router

unlink-lib: ## Unlink the @keenmate/svelte-spa-router library
	npm unlink @keenmate/svelte-spa-router

install-published: ## Install published version of @keenmate/svelte-spa-router
	npm install @keenmate/svelte-spa-router@latest
...
build-lib: ## Build the linked svelte-spa-router library
	cd ../svelte-spa-router && npm run package
...
docs-check: ## Check if all documentation pages are accessible
	...
	@echo "- http://localhost:$(DEV_PORT)/features/routing-modes"
	@echo "- http://localhost:$(DEV_PORT)/features/route-configuration"
	@echo "- http://localhost:$(DEV_PORT)/features/parameters"
	@echo "- http://localhost:$(DEV_PORT)/features/guards"
	@echo "- http://localhost:$(DEV_PORT)/features/named-routes"
	@echo "- http://localhost:$(DEV_PORT)/features/programmatic-navigation"
	@echo "- http://localhost:$(DEV_PORT)/features/querystring"
	@echo "- http://localhost:$(DEV_PORT)/features/filters"
	@echo "- http://localhost:$(DEV_PORT)/features/permissions"
	@echo "- http://localhost:$(DEV_PORT)/api"
...
status: ## Show project status
	@echo "Svelte SPA Router Showcase Status:"
	@echo "Node version: $(shell node --version)"
	@echo "NPM version: $(shell npm --version)"
	@echo "Project directory: $(shell pwd)"
	@echo "Library linked: $(shell npm list @keenmate/svelte-spa-router 2>/dev/null | grep @keenmate/svelte-spa-router || echo 'Not linked')"
